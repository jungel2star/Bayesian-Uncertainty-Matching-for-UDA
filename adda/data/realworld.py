...
@register_dataset('realworld')
class RealWorld(DatasetGroup):
    ...
    def __init__(self, path=None, shuffle=True):
        DatasetGroup.__init__(self, 'realworld', path=path)
        self.image_shape = (256, 256, 3)
        self.label_shape = ()
        self.shuffle = shuffle
        self._load_datasets()
    def _load_datasets(self):
        dataset,label=pickle.load(open("./data/officehome/realworld_256.pkl","rb"))
        train_images =dataset
        train_labels =label

        validation_images, validation_labels = pickle.load(open("./data/officehome/realworld_256.pkl", "rb"))
        test_images = []
        test_labels = []
