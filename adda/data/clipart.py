...
@register_dataset('clipart')
class Clipart(DatasetGroup):
    ...
    def __init__(self, path=None, shuffle=True):
        DatasetGroup.__init__(self, 'clipart', path=path)
        self.image_shape = (256, 256, 3)
        self.label_shape = ()
        self.shuffle = shuffle
        self._load_datasets()
    def _load_datasets(self):
        dataset,label=pickle.load(open("./data/officehome/clipart_256.pkl","rb"))
        train_images =dataset
        train_labels =label

        validation_images, validation_labels = pickle.load(open("./data/officehome/clipart_256.pkl", "rb"))
        test_images = []
        test_labels = []
